...
open class TZSegmentedControl: UIControl {
    ...
    public var edgeInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
  ...
  override open func draw(_ rect: CGRect) {
    ...
    if self.type == .text {
      ...
      for (index, image) in self.sectionImages.enumerated() {
        ...
        let imageyOffset : CGFloat = CGFloat(roundf(Float(
          ((self.frame.height - self.selectionIndicatorHeight) / 2) + 8.0 - 34.0)))
        
        let imageRect = CGRect(x: imagexOffset, y: imageyOffset, width: imageWidth, height: imageHeight)
        var textRect = CGRect(x: textxOffset, y: yOffset, width: textWidth, height: stringHeight)//Brijesh
        ...
        //                imageLayer.transform = CATransform3DMakeScale(0.40, 0.40, 1);
    ...
    private func frameForSelectionIndicator() -> CGRect {
        ...
        if self.type == .text {
            ...
            sectionWidth = self.frame.size.width / CGFloat(sectionTitles.count)//self.measureTitleAtIndex(index: self.selectedSegmentIndex).width
        } else if self.type == .images {
